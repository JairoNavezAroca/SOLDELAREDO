...
<?php 
	...
	if (isset($_GET['id']) && (isset($_GET['Word']) || isset($_GET['PDF']))) {
		...
		$der = DetalleEvRendimiento::find($_GET['id']);
		$er = EvRendimiento::find($der->IdEvRendimiento);
		$p = Personal::find($er->DniPersonal);
	}
	else{
...
<div id="page-content" hidden="">
	...
	<div style="text-align: center;">
		Evaluación de Rendimiento a <?php echo "$p->apellido, $p->nombre" ?>
	</div>
	Fecha: <?php echo date_format(date_create($der->Fecha),'d-m-Y h:i:s a') ?>
	<br>
	Porcentaje: <?php echo $der->Porcentaje ?>%
	<br>
	Observacion: 
	<br>
